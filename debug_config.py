...
class DebugLogger:
    ...
    def print(self, tag_or_message, message_or_args=None, *args):
        ...
        if message_or_args is not None:
            ...
            print(f"[DEBUG {self.tag}:{sub_tag}] {formatted_message}")
        else:
            # Called without sub-tag: logger.print("message")
            message = tag_or_message
            formatted_message = message % args if args else message
            print(f"[DEBUG {self.tag}] {formatted_message}")
    ...
    def log_pose_data(self, pose_data, source="unknown"):
        ...
        if DEBUG_ENABLED and self.enable and pose_data:
            print(f"[DEBUG POSE {source}] Label: {pose_data.get('raw_features', {}).get('label', 'N/A')}")
            print(f"[DEBUG POSE {source}] Torso angle: {pose_data.get('raw_features', {}).get('torso_angle', 'N/A')}")
            print(f"[DEBUG POSE {source}] Thigh uprightness: {pose_data.get('raw_features', {}).get('thigh_uprightness', 'N/A')}")
