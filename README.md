...
This project is one component of a four-part distributed system:

| Camera (MaixCAM)                                                                                              | Analytics Server (Third-party)                                                            | Streaming Server                                                                                                                                                                                                                                               | Caregiver UI (Web Dashboard)                                                                                                                  |
| :------------------------------------------------------------------------------------------------------------ | :---------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------- |
| • Pose Estimation<br>• Fall Detection<br>• Tracking<br>• Feature Extraction & Encryption<br>• Safety Checking | • Encrypted Features Comparisons<br>• Polynomial Evaluation of Encrypted Features<br>• Pose Classification | • System Orchestration<br>• Camera Registry & Management<br>• Data & Command Routing<br>• Background Image Storage<br>• Analytics Server Communication<br>• HTTP Streaming Backend<br>• Alert Generation (Telegram Bot) | • Live Streaming<br>• Camera Configuration<br>• Camera Management<br>• Alert & Notification Interface<br>• Patient Monitoring Dashboard |

### Component Overview

| Component            | Repository                                                                             | Description                                                                                                                                                                                                                                                                                                                                                                    | Status   |
| -------------------- | -------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------- |
| **Camera**           | [private-cctv](https://github.com/yudhisthereal/Private-Surveillence-MaixCAM)          | Edge device that performs pose estimation, fall detection, feature extraction, encryption, and privacy protection.                                                                                                                                                                                                                                                             | ✅ Active |
| **Analytics Server** | [HME pose classification](https://github.com/yudhisthereal/fall-detection-analytics/)  | Third-party server that performs encrypted features comparisons and polynomial evaluation on encrypted features to enable privacy-preserving pose classification without accessing raw data.                                                                                                                                                                                    | ✅ Active |
| **Streaming Server** | [fall-detection-streaming](https://github.com/yudhisthereal/fall-detection-streaming/) | Central orchestrator that serves the Caregiver UI, manages camera registration and metadata, stores background images, routes commands and status updates between the Camera and Caregiver, communicates with the Analytics Server for encrypted pose classification, and generates alerts via [Telegram Bot](https://t.me/elderly_patient_monitoring_bot) when incidents occur. | ✅ Active |
| **Caregiver UI**     | *(Part of the Streaming Server repository)*                                            | Browser-based dashboard that allows caregivers to monitor live streams, receive alerts, manage cameras, and remotely configure camera settings.                                                                                                                                                                                                                                | ✅ Active |
...
- Recovery is accepted only when:
    ...
    2. Bounding-box bottom alignment is close enough:
     $|y_{bottom}^{now} - y_{bottom}^{snapshot}| \le POSE\_RECOVERY\_BBOX\_BOTTOM\_TOLERANCE\_PX$
