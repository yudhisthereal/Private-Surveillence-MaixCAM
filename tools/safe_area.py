...
class BodySafetyChecker:
    ...
    def body_in_safe_zone(self, 
        ...
        """
        Check if body keypoints are inside any safe polygon based on check method
        ...
        if not self._safe_polygons:
            return True  # No safe polygons defined = everywhere is safe
        
        # Filter valid keypoints (confidence > 0.1)
        valid_keypoints = [(x, y) for x, y, conf in body_keypoints if conf > 0.1 and x > 0 and y > 0]
        
        if not valid_keypoints:
            return True  # If no valid keypoints, assume safe
        ...
        if not points_to_check:
            return True  # If no points to check, assume safe
        
        # Check if all points are inside any of the safe polygons
        for point in points_to_check:
            point_inside_any_polygon = False
            
            for polygon_coords in self._safe_polygons:
            ...
            # If any required point is not inside any safe polygon, body is not safe
    ...
    def get_containing_polygons(self, 
        ...
        if not self._safe_polygons:
            return []  # No safe polygons defined
        ...
        safe_polygon_indices = []
        
        for poly_idx, polygon_coords in enumerate(self._safe_polygons):
            polygon_points = [Point(x, y) for x, y in polygon_coords]
            ...
            if all_points_inside:
                safe_polygon_indices.append(poly_idx)
        
        return safe_polygon_indices
    ...
    def is_point_safe(self, point: Tuple[float, float]) -> bool:
        """
        Check if a single point is inside any safe polygon
        
        Args:
            point: (x, y) coordinates
        
        Returns:
            bool: True if point is inside any safe polygon
        """
        if not self._safe_polygons:
            return True  # No polygons = everywhere is safe
            
        point_obj = Point(point[0], point[1])
        
        for polygon_coords in self._safe_polygons:
            polygon_points = [Point(x, y) for x, y in polygon_coords]
            if self.point_in_polygon(point_obj, polygon_points):
                return True
